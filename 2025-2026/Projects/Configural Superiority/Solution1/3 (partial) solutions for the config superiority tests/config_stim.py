...
# this script illustrates how to show a stimulus for limited time while simultaneously measuring RT
...
for trial in trials:
    ...
    if speedy:
        ...
        keys = None
        flipped = False # don't keep on flipping
        while (my_clock.getTime() < max_waiting_time) and not keys:
            if (stimulus_timer.getTime() > presentation_time) and not flipped:
                win.flip()
                flipped = True 
            keys = event.getKeys(keyList = response_options)
    RT = my_clock.getTime()
    if not keys:  # keys[0]
