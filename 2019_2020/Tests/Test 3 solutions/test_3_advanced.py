...
for b in range(nBlocks):
    ...
    for i in range(b*nBlockTrials,(b+1)*nBlockTrials):
        ...
        if trials[i,9].astype(int)==1:
            deadline=deadline-.05
        else:
            deadline=deadline+.05
    ...
    print(trials[currentTrials,9].astype(int))
    Block_accuracy  =numpy.mean(trials[currentTrials,9].astype(int))*100
    Block_feedback  = visual.TextStim(win, text = "Je behaalde in het laatste blok een accuraatheid van {0} %".format(Block_accuracy))
