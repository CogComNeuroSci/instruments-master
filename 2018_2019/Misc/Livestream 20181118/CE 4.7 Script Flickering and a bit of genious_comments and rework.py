...
## Esther: better not to work in a full screen window if there is a while loop involved
win = visual.Window(fullscr = True , color = 'black', units = 'norm', monitor = mon)

## Esther: you were looking for an escape function to get out of an infinite loop?
for key in ['q', 'escape']:
...
while counter < 100:
    ...
    ## Esther: this is a very nice find!
    graf.autoDraw = True
    graf2.autoDraw = True
    graf3.autoDraw = True
    graf4.autoDraw = True
    ...
    win.flip()
    time.sleep(duration)
    ## Esther: this is why two lines don't more reightward.
    rotaties2 += 1
    rotaties += 1
    ## Esther: the code below is the reason for the flicking
#    win.flip()
#    time.sleep(duration)
