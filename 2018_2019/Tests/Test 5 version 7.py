...
novel in version 7: randomization with arrays
"""
...
def shuffle(a_list, n_rep):
    full_array = np.tile(a_list, (n_rep, 1))
    violation = True
    while violation:
        np.random.shuffle(full_array)
        check = True
        for loop in range(len(full_array)-1):
            if np.all(full_array[loop] == full_array[loop+1]):
                check = False
                break
        violation = not check
    # and finally, make a list of dicts for the trialhandler
    full_list = []
    for loop in range(full_array.shape[0]):
        prime_this_trial  = stimuli[int(full_array[loop,0])]
        target_this_trial = stimuli[int(full_array[loop,1])]
        full_list.append(OrderedDict({"Prime": prime_this_trial, "Target": target_this_trial}))
    return full_list

# make basic conditions vector; with arrays
p = p_con*(1-info_subject["Participant nummer"]%2) + (1-p_con)*(info_subject["Participant nummer"]%2)
n_prime  = len(stimuli)
n_target = len(stimuli)
n_unique = n_prime*n_target
Unique   = np.array(range(n_unique))
Prime    = np.floor(Unique/n_target)
Target   = np.floor(Unique/1)%n_target
basic_conditions = np.column_stack((Prime, Target))
conditions1 = np.tile([basic_conditions[0], basic_conditions[3]], (int(np.floor(p/2*len(basic_conditions)*n_reps)),1))
conditions2 = np.tile([basic_conditions[1], basic_conditions[2]], (int(np.floor((1-p)/2*len(basic_conditions)*n_reps)),1))
conditions = np.row_stack((conditions1, conditions2))
# worth checking in this case
conditions_pd = pd.DataFrame.from_records(conditions)
conditions_pd.columns = ["Prime", "Target"]
...
for block in range(n_blocks):
    block_nr += 1
    shuffled_conditions = shuffle(conditions, n_rep = 1)
