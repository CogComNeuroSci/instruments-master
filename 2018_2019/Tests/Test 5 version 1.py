...
# Experiment length
n_blocks       = 3
n_reps         = 1 # must be 6 in exp, can be lower for testing
...
for block in range(n_blocks):
    ...
    for trial in trials:
        ...
        target.draw()
        win.flip()
        core.wait(dur_target)
        win.flip() # remove target after presentation time
        
        # version 1: RT measurement starts after target presentation
        RT_clock.reset()
