...
trials      = numpy.column_stack([Subject, Gender, Age, Handedness, OrientationTrials, SpacialFrequencyTrials, CorResp, RT, Resp, Accuracy, PresentationTimeCheck, PresentationTimeTrials])
...
trials[:,11] = numpy.repeat(PresentationTime, nOrientations*nSpacialFrequencies*nreps)
...
for b in range(nblocks):
    ...
    for i in range(b*ntrials,(b+1)*ntrials):
        ...
        core.wait(trials[i,11].astype(numpy.float)/1000)
        ...
        # determine accuracy
        trials[i,9] = int(trials[i,8] == allowedKeys[int(trials[i,6])])
...
pylab.plot(trials[:,10].astype(numpy.float)*1000, '-')
