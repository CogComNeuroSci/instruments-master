...
## Esther: we hadden liever dat je de coordinaten van de maan bekomt via code in plaats van via een expliciete optelling van getallen
...
## Esther: hier heb je een nul te veel geplaatst waardoor de maan tien keer te klein is
...
for i in range(len(planeet_x)):
    ## Esther: dit is inderdaad wat we op het oog hadden
...
## Esther: let er op dat voor puur geel de code voor blauw best op -1 staat in plaat van 0
...
## Esther: je hoefde niet perse alle waarden van de radius bij te houden, maar het is ook niet fout als je dat wel doet
...
for step in range(60):
    ...
    ## Esther: het is nog beter om de stimulus op voorhand aan te maken en in de loop dan enkel de positie, radius en kleur te updaten in de loop
...
#    
## Esther: je moet eerst checken of beide objecten de zon hebben geraakt, pas daarna kan je kijken naar de beiden apart, anders komt deze boodschap nooit op het scherm
