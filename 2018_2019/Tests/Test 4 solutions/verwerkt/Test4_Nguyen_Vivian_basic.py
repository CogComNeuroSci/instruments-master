...
Orientations = ["<", ">"]
Positions = ["pos = (-0.5, 0)", "pos = (0,0)", "pos = (0.5, 0)"]
...
for blocki in range(nBlocks):
    ...
    for trial in TrialsBlock:
        ...
        Stimulus.position = trial["PositionArrow"]
        Stimulus.draw()
        win.flip()
        ...
        my_clock.reset()
        keys = event.waitKeys(keyList = ["curses.KEY_LEFT", "curses.KEY_RIGHT", "curses.KEY_DOWN"])
        
        TrialsBlock.addData("response", keys[0])
        accuracy = 1*(keys[0] == correct [trial["Number"]-1])
        TrialsBlock.addData("Acc", accuracy)
        TrialsBlock.addData("RT", my_clock_getTime())
