...
# Esther: laat geen puntjes liggen door bv. third gender te vergeten ;)
...
# Esther: vergeet er bij de goodbye message niet voor te zorgen dat de proefpersoon weet op welke toets ze moeten drukken                                    
...
# Esther: die is niet helemaal het design dat je zou willen: enkel de eerste twee posities worden gebruikt
...
for blocki in range(nBlocks):
    ...
    # Esther: dat is een goede oplossing, maar vergeet niet om ook een optie te voorzien voor de middelste positie
    if (blocki)%3 == 0:
...
#    loop over de geshuffelde trials voor dat block
    for trial in blockTrials:
        ...
        win.flip()
        rtclock.reset()
        # Esther: dit is niet de correcte code om de keylist in te geven!
        resp= event.waitKeys("f", "j")
        RT= clock.getTime()
        
        # Esther: pas op, je had dit rtclock genoemd!
        
#        ##store rt
#        trials[currentTrials, 9]=
        
        # Esther: dit was inderdaad niet de correcte manier geweest om de response gegevens op te slaan
    ...
    # Esther: via dit soort vergelijkingen kan je enkel de congruentie van individuele trials bepalen, maar niet van een reeks trials
