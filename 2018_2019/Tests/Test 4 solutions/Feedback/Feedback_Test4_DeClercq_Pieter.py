...
while already_exists:
    ...
    # Esther: hier hadden we eigenlijk gevraagd naar een folder met de naam data, een folder die desnoods automatisch aangemaakt wordt als ze nog niet bestaat.
    ...
    ## Esther: haha, dat is heel anoniem, maar je slaat enkel een paar strings op zonder bijhorende antwoorden ;)
...
def message(message_text = "", duration = 0, height = None, pos = (0.0, 0.0), color = "white"):
    ...
    if duration == 0:
        ...
        if x[0] in ['escape', 'esc']:##dit is nergens gevraagd, maar steek ik er tussen voor mijn gemak. Ik gebruik ook win.close omdat dit vlotter is dan break
            ...
            ## Esther: je mag dit gerust doen, maar het nadeel van win.close() is dat dit heel je experiment stopt zonder dat de data weggeschreven kunnen worden
            ## Esther: het is maar dat je je goed bewust bent van de implicaties van deze aanpak ;)
...
## Esther: misschien toch een goed idee om deze tekst ook op te splitsen in verschillene regels voor de leesbaarheid van het script ;)
...
## Esther: een nogal hard-coded oplossing, but it does the trick
...
for blok in range(12):
    ...
    ## Esther: fullRandom was hier nog beter dan random
    ...
    for trial in trials:
        ...
        if ((trial["Pijltje"]=="<") and trial["Positie"]==(-0.5,0.0)) or ((trial["Pijltje"]==">") and trial["Positie"]==(0.5,0.0)):
            ...
            ## Esther: de logische vergelijking hierboven kon je nog gemakkelijker definiëren want het pijltje maakt eigenlijk niet uit
...
## Esther: zorg dat het hier ook duidelijk is op welke response toets de proefpersonen kunnen duwen
