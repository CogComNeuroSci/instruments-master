...
while already_exists:
    ...
    file_name = directory_to_write_to + "/Test4_subject_" + str(info["Participant number"]) + "_Anonymity3"
...
for trial in trials:
    ...
    trials.addData("Participant number", info["Participant number"])
    trials.addData("Age", info["Age"])
    trials.addData("Gender", info["Gender"])
    trials.addData("Handedness", info["Handedness"])
