...
while trialcounter < trials.shape[0]:
    ...
    if trials[trialcounter,7] == 1:
        if trials[trialcounter,5] == 0:
            instruct_dir.draw()
        else:
            instruct_pos.draw()
        win.flip()
    ...
    stimulus.pos = (PositionOptions[int(trials[trialcounter,1])],0)
    stimulus.text = ArrowOptions[int(trials[trialcounter,0]) ]
    stimulus.draw()
    win.flip()
    ...
    CorResp = RespOptions[int(trials[trialcounter,6])]
    accuracy = (keys[0] == CorResp) * 1
    
    ## store the response information
    if keys[0] == "left":
        trials[trialcounter,8] = 0
    elif keys[0] == "down":
        trials[trialcounter,8] = 1
    else:
        trials[trialcounter,8] = 2
    
    trials[trialcounter,9]     = accuracy
    trials[trialcounter,10]    = RT
