...
DesignLD        = data.createFactorialTrialList ({"Arrow": ArrowOptions, "Position": PositionOptions})
...
for blocki in range(nBlocks):
    ...
    for trial in trials:
        ...
        if trial['Position'] == 0:
            Congruence = "neutral"
        elif (trial['Arrow'] == "<" and trial['Position'] == -0.5) or (trial['Arrow'] == ">" and trial['Position'] == 0.5):
        ...
        if BlockTypes[blocki] == 0:
            if trial['Arrow'] == "<":
                CorResp = RespOptions[0]
            else:
                CorResp = RespOptions[1]
        else:
            if trial['Position'] == -0.5:
                CorResp = RespOptions[0]
            elif trial['Position'] == 0.5:
                CorResp = RespOptions[1]
            else:
                CorResp = RespOptions[2]
        ...
        trials.addData("congruence", Congruence)
