...
# maak er arrays van
### Esther: hier hadden we graag dat alle eigenschappen van de proefpersoon geregistreerd zouden worden (behalve de naam)
### Esther: waarom niet meteen de 
...
def feedback_message():
    if keys[0] != 0:  
        ### Esther: de correcte response wordt nergens echt bepaald
...
for b in range(nblocks):
    ...
    for i in range(b*ntrials,(b+1)*ntrials):
        ...
        ### Esther: dit is een meer optimaal moment om het keyboard te resetten
        
        gabor.draw()
        win.flip()
        
        ### Esther: hier moet de klok gereset worden
        
        core.wait(0.55)#hier zou ik 'display_tijd[i]' zetten, maar er is foutmelding
        
        ### Esther: hier moest je de index b gebruiken gezien het bloknummer de aanbiedingstijd bepaalt
        ...
        ### Esther: oeps, escape behoort niet tot de response mogelijkheden
        keys=event.waitKeys(keyList = ["j", "f"])
        
        ### Hier zou ik meteen de RT al registreren
