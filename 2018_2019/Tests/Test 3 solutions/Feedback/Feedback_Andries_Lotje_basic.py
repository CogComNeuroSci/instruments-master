...
n_blocks=3
### ESther: pas op, 8 trials!
n_trials = 3
### Esther: pas op, deze duraties zijn 10 keer te lang!
Gabor_duration = [0.16,0.33,0.50]
### Esther: het is niet nodig om een tuple te maken hieronder
...
### Esther: deze mask kan je meteen al invoegen bij eht aanmaken van de gabor
grating.mask = "gauss"

#Feedback
def Feedback_message():
    ### Esther: CorResp moet nog vergeleken worden met iets...
    if CorResp:
...
for b in range (n_blocks):
    ...
    for i in range(b*n_trials,(b+1)*n_trials):
        ...
        grating.draw()
        win.flip()
        ### ESther: dit is het moment om de klok te herzetten
        ### Esther: core.wait!
        ...
        ### Esther: dit is een nog beter moment om het keyboard te clearen
        win.flip()
        #reset clock
        myClock.reset()
        #clear keyboard
        ### Esther: waarom twee keer wachten op een response?
...
#       #correct response
#        if grating.ori == 45.0:    # Esther: waarom deze orientatie?
#           CorResp= "j"
#        if grating.ori == 135.0:   # Esther: waarom deze orientatie?
#           CorReso="f"             # Esther: typo
#
#        print feedback
#        Feedback_message()

    ### Esther: eerst uit de trial loop breken, dan pas uit de blok loop breken
