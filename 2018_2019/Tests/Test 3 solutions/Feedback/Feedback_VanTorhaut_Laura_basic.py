...
### Esther: pas op, je kruist de eigenschappen van de trials niet
...
welkom.draw ()
win.flip ()
### Esther: het is niet nodig om van die "space" een list te maken
...
for i in range (n_trials):
    ...
    ### Esther: het is beter om de stimuli aan te maken buiten de loop en enkel de relevante eigenschappen aan te passen binnen de loop1
    stimulus = visual.GratingStim (win, mask = "circle", ori = oriëntatie [i], sf = spat_frequentie [i])
    stimulus.draw ()
    ### Esther: dit is niet de juiste plaats om de klok te resetten
    mijn_klok.reset()
    ### Esther: pas op, haakjes enzo niet vergeten!
    win.flip
    ### Esther: pas op, deze aanbiedingstijd is 10 keer te lang
    core.wait (0.16)
