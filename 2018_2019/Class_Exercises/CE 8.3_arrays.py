...
Design = numpy.column_stack([ColorWord, FontColor, UniqueTrials])
...
for blocki in range(nBlocks):
    ...
    while stopcriterium != 1:
        ...
        comparison = numpy.diff(blockTrials[:,2])
    ...
    trials[currentTrials, 3] = blocki+1
    
    ## store the instructions
    trials[currentTrials, 4] = (blocki+1)%2
    
    ## store the correct response
    if (blocki+1)%2 == 0:
        trials[currentTrials, 5] = trials[currentTrials, 0]     ## correct answer determined by color word
    else:
        trials[currentTrials, 5] = trials[currentTrials, 1]     ## correct answer determined by font color
...
## export
trials.to_csv(path_or_buf = "CE8_3_output_arrays.csv", index = False)
