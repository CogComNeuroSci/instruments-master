...
## make trial matrix
trials = numpy.tile(blockTrials, (nBlocks, 1))

## fill in the block number (starting from 1 instead of 0)
blocknr = numpy.repeat(numpy.array(range(nBlocks)), blockTrials.shape[0])

## add the block numbers to the trial matrix
trials = numpy.column_stack([trials,blocknr])
...
## export
trials.to_csv(path_or_buf = "CE8_1_output_arrays_tilestructure.csv", index = False)
