...
# illustration that trialhandlers cannot be recycled
# try putting trials = data.Trialhandler() outside the block loop
# the program will crash the second time you want to go over the trials
# because trialhandlers cannot be recycled.
# in contrast, targets_responses is a list of dictionaries, so that CAN
# be outside the block_loop
# written by Tom Verguts, July 2023

from psychopy import data, visual, event, core
import os

data_file = os.path.join(os.getcwd(), 'data', 'experimental_data')
thisExp = data.ExperimentHandler(dataFileName=data_file)

visual_targets = [digit for digit in range(1,5)]
targets_responses = []
