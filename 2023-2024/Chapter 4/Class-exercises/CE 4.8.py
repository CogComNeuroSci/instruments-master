...
# nov 2020 addition: I made the code a bit cleaner
...
step_size = 0.02*10 ## 10% of screen
step_time = 0.1
...
trial_text = visual.TextStim(win,text="nothing yet")
...
for trial in range(n_trials):
    ...
    trial_text.text = "trial " + str(trial+1) + "!"
    trial_text.draw()
    win.flip()
    sleep(0.5)
    ...
    while (crash == False) and (time < max_time):
        ...
        car_blue.pos[car_blue.pos<=-1]   = -1
        car_green.pos[car_green.pos<=-1] = -1
        car_blue.pos[car_blue.pos>=+1]   = +1
        car_green.pos[car_green.pos>=+1] = +1
        ...
        ## Escape function to get out of infinite while loop. Just an extra; we know you have not seen this yet
