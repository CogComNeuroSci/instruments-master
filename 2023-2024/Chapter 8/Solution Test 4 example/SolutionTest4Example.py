# Test 4
# by Tom Verguts, March 2022
# note: I switched from CamelBackNotation to underscore_notation for variables 
# I think this is easier to read and less error-prone, but you are totally free to use either
...
directory_to_write_to      = my_directory
...
while already_exists:
    ...
    my_dlg = gui.DlgFromDict(dictionary = info, title = "Test 4")
...
mapping = int(subject_name[0] in ["a", "b", "c"])
...
n_letters  = len(letter_options)
n_colors   = len(color_options)
n_locations= len(location_options)
...
n_unique        = n_letters * n_colors * n_locations
n_block_trials  = n_unique * n_reps
unique_trials    = numpy.array(range(n_unique)) 
...
trial_list = pandas.DataFrame.to_dict(trials_DF, orient = "records")
