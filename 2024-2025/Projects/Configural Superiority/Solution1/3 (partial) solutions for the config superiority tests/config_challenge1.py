# The configural superiority task
# with a staircase to reach 70%
# note 1: you can use the stairhandler object type from psychopy,
#but I prefer to code it "by hand" to have better insight what goes on.
# note 2: A more traditional approach toward the staircase would be the
# N-down 1-up procedure (Leek, 2001, Perception & Psychophysics), but
# the assignnment asks us to use the last 10 trials.
# note 3: stimulus is not presented for 200 msec here: see config.stim.py for how to do that properly
...
initial_max_waiting_time  = 2        # start from this waiting time to change the staircase
jump_start, jump_size, jump_criterion = 5, 0.1, 0.7 # when to start staircasing, jump size (in seconds) to adapt the staircase; jump_criterion (in accuracy)
...
directory_to_read_from = os.path.join(my_directory, "stimuli") # alternative approach to define paths w.r.t. what we saw in class
...
trials          = numpy.ones((n_trials, n_columns_block + 3)) * numpy.nan # We add Block (1), Cor_resp (2), and Delay (3)
...
trials_DF.columns = ["stimuli", "block", "cor_resp", "delay"]
...
max_waiting_time = initial_max_waiting_time # initialize the staircase
accuracy_total = []
trial_count = 0
...
for trial in trials:
    ...
    trials.addData("WaitTime", max_waiting_time)
    this_exp.nextEntry()
    accuracy_total.append(accuracy) # unfortunately, with the trialhandler not so easy to access earlier data.. so I define an extra variable accuracy_total
    # adapt the staircase
    if trial_count > jump_start:
        accuracy_last = numpy.mean(accuracy_total[trial_count-jump_start:trial_count])
        max_waiting_time += (1-2*int(accuracy_last > jump_criterion))*jump_size # if statements are slow, I try to avoid them..
