...
DirectoryToWriteTo = os.getcwd() + '/TriadTaskData/' #os.getcwd() is where your script is saved
if not os.path.isdir(DirectoryToWriteTo):
    os.mkdir(DirectoryToWriteTo) #Create data folder (if this does not yet exist) 
...
PercentageRepeatsAllowed = 20 #How much percent of overlap is allowed between the current and previous trial? Overlap means that at least 2 words are similar
AllowedResponses.append('escape') #Just to make sure that escape can be used for quitting the task

WordList = ['apple','mouse','house','fort','head','horse','client','hit','sleeve']
TripletsList = []
for index0 in WordList: #The three for-loops make sure that we fully cross all words
    for index1 in WordList:
        for index2 in WordList:
            if not index0 == index1 and not index0 == index2 and not index1 == index2: #We don't want word repetitions within a single trial
                if not [index0,index1,index2] in TripletsList and not [index1,index2,index0] in TripletsList \
                and not [index2,index1,index0] in TripletsList and not [index0,index2,index1] in TripletsList \
                and not [index2, index0, index1] in TripletsList and not [index1,index0,index2] in TripletsList: #All these if-statements are to ensure that we don't have the same trial in the list twice (also not with changed word order)
                    TripletsList.append([index0,index1,index2]) #The order of the words within a trial will be randomized in the trial loop later

TrialMatrix = numpy.tile('EmptyCell', (len(TripletsList)*NrOfBlocks,len(TripletsList[0])+2)) #Just placeholders for now... The length of the first element is the same as the nr of columns of the list

for BlockNr in range(NrOfBlocks): #Here we fill an empty matrix with all trials of the experiment

    while True:
        numpy.random.shuffle(TripletsList) #All lines are shuffled
        SimilarityCounter_AllLinesConsideration = 0

        for line in range(1, len(TripletsList)): #Checking LINE BY LINE of TripletsArray
            SimilarityCounter_TwoLinesComparison = 0

            for coordinate in [[0,0],[0,1],[0,2],[1,0],[1,1],[1,2],[2,0],[2,1],[2,2]]: #We compare the first coordinate (coordinate[0]) of the current line (line) to the second coordinate (coordinate[1]) of the previous line (line-1)
                if TripletsList[line][coordinate[0]] == TripletsList[line-1][coordinate[1]]:
        ...
        if SimilarityCounter_AllLinesConsideration <= (len(TripletsList) / (100/PercentageRepeatsAllowed)): #Only escape the while loop if there is less than e.g., 20% repetition of at least 2 words between current and previous lines (for 20%, for stopping it should be below 16.8 repetitions, so 16 is max)
            break

    TrialMatrix[BlockNr * len(TripletsList) : (BlockNr + 1) * len(TripletsList), 0: len(TripletsList[0])] = TripletsList
    TrialMatrix[BlockNr * len(TripletsList) : (BlockNr + 1) * len(TripletsList), len(TripletsList[0])] = BlockNr+1 #+ 1 because Pyton starts at 0
    TrialMatrix[BlockNr * len(TripletsList) : (BlockNr + 1) * len(TripletsList), len(TripletsList[0])+1] = numpy.arange(len(TripletsList))+1 #The trial nr
...
for trial in trials:
    ...
    if (trials.thisN+1) % len(TripletsList) == 0: #To decide when a break is offered to the participant
        if (trials.thisN+1)/len(TripletsList) == NrOfBlocks:
