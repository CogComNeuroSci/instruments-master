...
Correct = numpy.array([int(i%8 + 1) for i in range(Nunique)]) 
print(Correct)
Accuracy = numpy.repeat(-99, Nunique)
TrialsMatrix = numpy.column_stack([Stimuli, Answers, RT, Correct, Accuracy])
#Step 2: Pandas dataframe
TrialsDataFrame = pandas.DataFrame.from_records(TrialsMatrix)
TrialsDataFrame.columns = ['Question', 'Answer', 'RT', 'Correct', 'Accuracy']
...
Ndone = 0 #counter to save the number of answered questions
for trial in trials:
