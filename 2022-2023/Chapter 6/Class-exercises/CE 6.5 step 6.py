...
for b in range(nblocks):
    ...
    averageRT   = numpy.nanmean(trials[range(b*ntrials,(b+1)*ntrials),6].astype(numpy.float))*1000
    averageACC  = numpy.nanmean(trials[range(b*ntrials,(b+1)*ntrials),5].astype(numpy.float))*100
