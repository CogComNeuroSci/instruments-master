...
participant_name = "Vincent Hoofs"
...
#Making window
win = visual.Window([700, 700], color = (1,-1,1), units ='norm')
...
Circle_blue = visual.Circle(win, size = .1, fillColor = (-1, -1, 1))
Square_blue = visual.rect.Rect(win, size = .1, fillColor = (-1, -1, 1))

#Feedback stimulus
Feedback = visual.TextStim(win, text = "Too late!", color = (-1,-1, -1))

#Gather all design variables in list
sizes = 2*np.arange(.01, .16, .01)
stimuli = [Circle_red, Square_red, Circle_blue, Square_blue]
all_stimuli = np.repeat(stimuli, Nblocktrials/len(stimuli))
