...
while trial_index < len(data):
    ...
    if trial_index == 0 or len(rt)%10 == 0:
        mean = sum(rt)/len(rt)
        meanRT.append(mean)

    BlockRT = meanRT[-1]
    print('Block mean RT is %d') %BlockRT
    ...
    while len(rt)%10 != 0:
        if len(rt) < 10:
            ...
            if reactiontime > (sum(rt)/len(rt)):
                my_window.flip(clearBuffer=True)
                instructions = "Please answer faster! \n \nYou were too slow! \n \nPress any key to continue."
    ...
    if answer[0] in ['left','right']:
        ...
        experiment_data.append([congruency, which_cue, target_loc,answer[0],accuracy[0],reactiontime,BlockRT])
        ...
        print (type(trial_parameters[trial_index])), (trial_parameters[trial_index])
        experiment_data.append([trial_parameters[trial_index], which_cue, target_loc,answer[0],accuracy[0],reactiontime,BlockRT])
        event.waitKeys()
...
expData = pd.DataFrame(experiment_data, columns = ['Congr', 'Cue','Target','Key','Acc','RT (in ms)','Block RT'])
print (expData)

expData.to_csv("PosnerExp_Subject_%02d.txt" %subject_id, sep = '\t')
